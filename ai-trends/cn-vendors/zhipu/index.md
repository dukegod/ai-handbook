...
| 模型 | 定位 | 上下文 | 思考模式 | AA 智能分 | 主要场景 |
|------|------|--------|----------|----------|----------|
| **GLM-5.3** | 新旗舰 | 1M | — | **60** | Agentic Engineering / 通用（后训练 Scaling 旗舰） |
| **GLM-5.2** | 旗舰 | 1M | — | 53 | Agentic Engineering / 通用 |
| **GLM-5** | 上一代旗舰 | 200K | — | — | 已由 5.2 取代（744B / 40B） |
| **GLM-Z1** | 推理增强 | — | RLVR | — | 数学 / 代码 / 逻辑 |
| **GLM-4-Air** | 轻量 | — | — | — | 高并发 / 低成本 |

> **产品线逻辑**：GLM-5.3 是最新旗舰（AA 60 分，与 Kimi K3 并列开源最高），GLM-5.2 在售（AA 53），GLM-5 是上一代，GLM-Z1 做推理，GLM-4-Air 做轻量。

> **关键洞察**：GLM-5.3 与 5.2 据称共享同一预训练基座，7 分差距（53→60）**纯靠后训练 Scaling** 实现——这验证了"后训练是当前最大杠杆"的行业趋势。
